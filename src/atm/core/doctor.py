"""`atm doctor` — audit drift across all configured roots.
...
from atm.core.models import RegisteredRepo, RuleEntry
...
def _audit_project(root: Path, report: DoctorReport) -> None:
    ...
    try:
        ...
        report.findings.append(Finding("warning", root, "no .atm/manifest.json (not initialized)"))
        return
