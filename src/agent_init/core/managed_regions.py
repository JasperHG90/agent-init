...
HTML_DIALECT = Dialect(
    name="html",
    begin=lambda n: f"<!-- BEGIN agent-init: {n} -->",
    end=lambda n: f"<!-- END agent-init: {n} -->",
    region_re=re.compile(
        r"<!-- BEGIN agent-init: (?P<name>[a-z0-9_-]+) -->"
        r"(?P<body>.*?)"
        r"<!-- END agent-init: (?P=name) -->",
        re.DOTALL,
    ),
    end_re=re.compile(r"<!-- END agent-init: ([a-z0-9_-]+) -->"),
)
...
def for_filename(filename: str) -> Dialect:
    ...
    if lower.endswith((".md", ".html", ".htm")):
        return HTML_DIALECT
    if lower.endswith((".yaml", ".yml", ".toml", ".ini", ".cfg", ".sh", ".gitignore", ".editorconfig")):
        return HASH_DIALECT
    return HTML_DIALECT
...
def parse(text: str, dialect: Dialect = HTML_DIALECT) -> list[Region]:
    regions = [
        Region(name=m.group("name"), body=m.group("body"))
        for m in dialect.region_re.finditer(text)
    ]
    ...
    if sorted(begins) != sorted(ends):
        raise RegionError(
            f"unbalanced agent-init markers ({dialect.name}): begins={sorted(begins)} ends={sorted(ends)}"
        )
    return regions
