...
HTML_DIALECT = Dialect(
    name="html",
    begin=lambda n: f"<!-- BEGIN atm: {n} -->",
    end=lambda n: f"<!-- END atm: {n} -->",
    region_re=re.compile(
        r"<!-- BEGIN atm: (?P<name>[a-z0-9_-]+) -->"
        r"(?P<body>.*?)"
        r"<!-- END atm: (?P=name) -->",
        re.DOTALL,
    ),
    end_re=re.compile(r"<!-- END atm: ([a-z0-9_-]+) -->"),
)
...
def parse(text: str, dialect: Dialect = HTML_DIALECT) -> list[Region]:
    ...
    if sorted(begins) != sorted(ends):
        raise RegionError(
            f"unbalanced atm markers ({dialect.name}): begins={sorted(begins)} ends={sorted(ends)}"
        )
    return regions
