...
`.atm/manifest.json`. Merge semantics preserve unmanaged servers and
top-level keys.
"""
...
from atm.core import hashing, manifest, mcp_registry, validation
from atm.core.models import InstalledMcpServer, Manifest, McpClaudeEntry
...
def _check_alias_available(
    ...
    if isinstance(servers, dict) and alias in servers:
        ...
        if managed is None and not force:
            raise McpAliasConflictError(
                f"alias {alias!r} already exists in .mcp.json (not managed by atm). "
