"""`atm init` orchestration: render AGENTS.md, optionally write mirror
...
from atm.core.models import Manifest
from atm.core.validation import MirrorNameError, is_valid_mirror_name
        ...
        if target.exists() and not target.is_symlink():
            ...
            if options.force:
                drift_warnings.append(
                    f"force-overwrote {target.name} (had no atm markers; "
                ...
                continue
            drift_warnings.append(
                f"{target.name} exists with no atm markers; left as-is (use --force to overwrite)"
            )
            mirror_paths.append(target)
            continue
...
def _detect_region_drift(filename: str, body: str, stored_hashes: dict[str, str]) -> list[str]:
    ...
    try:
        ...
        raise agents_md.RegionError(f"{filename}: malformed atm region markers — {exc}") from exc
