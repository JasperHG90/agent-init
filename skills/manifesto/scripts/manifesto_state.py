...
def _find_anchor(path: Path, anchor: str) -> list[int]:
    ...
    return [i + 1 for i, ln in enumerate(lines) if collapsed and collapsed in " ".join(ln.split())]
...
def _expand_tracked(root: Path, globs: list[str], enforcer_paths: list[str]) -> dict[str, str]:
    out: dict[str, str] = {}
    rels: set[str] = set()
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_resolve = sub.add_parser("resolve", help="find an anchor's current line in a file")
    p_resolve.add_argument("path")
    p_resolve.add_argument("anchor")
    p_resolve.set_defaults(func=cmd_resolve)

    sub.add_parser("write", help="recompute resolved lines + hashes").set_defaults(func=cmd_write)
    sub.add_parser("diff", help="list tracked files that changed").set_defaults(func=cmd_diff)
    sub.add_parser("verify", help="re-resolve every enforcer anchor").set_defaults(func=cmd_verify)
