...
_BEGIN = r"<!-- BEGIN agent-init: (?P<name>[a-z0-9_-]+) -->"
_END = r"<!-- END agent-init: (?P=name) -->"
...
def parse(text: str) -> list[Region]:
    ...
    regions = [Region(name=m.group("name"), body=m.group("body")) for m in _REGION_RE.finditer(text)]
    # Detect unbalanced markers: count BEGIN/END occurrences per name.
    begins = re.findall(_BEGIN, text)
    ends = re.findall(r"<!-- END agent-init: ([a-z0-9_-]+) -->", text)
    if sorted(begins) != sorted(ends):
        raise RegionError(
            f"unbalanced agent-init markers: begins={sorted(begins)} ends={sorted(ends)}"
        )
    return regions
...
def merge(existing: str, new_regions: dict[str, str]) -> str:
    ...
    def _replace(match: re.Match[str]) -> str:
        ...
        if not body.endswith("\n"):
            body = body + "\n"
        return f"<!-- BEGIN agent-init: {name} -->{body}<!-- END agent-init: {name} -->"
    ...
    if missing:
        ...
        for name, body in missing:
            ...
            if not body.endswith("\n"):
                body = body + "\n"
            suffix_parts.append(
                f"\n<!-- BEGIN agent-init: {name} -->{body}<!-- END agent-init: {name} -->\n"
            )
...
def build(regions: Iterable[tuple[str, str]]) -> str:
    ...
    for name, body in regions:
        ...
        if not body.endswith("\n"):
            body = body + "\n"
        parts.append(f"<!-- BEGIN agent-init: {name} -->{body}<!-- END agent-init: {name} -->")
    return "\n".join(parts) + "\n"
