...
Users can add their own via `atm template add` (CLI lands in a later
...
from atm.core.models import Template

BUILTIN_DEFAULT = "default"
_BUILTIN_PACKAGE = "atm.templates"
        ...
        if existing is None:
            session.add(
                Template(
                    ...
                    source="builtin",
                    description="Default atm template (managed regions for header + rules).",
                )
            )
            session.commit()
