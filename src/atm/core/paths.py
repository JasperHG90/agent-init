"""Platform-aware paths for atm's global state and per-project state.

Global state lives under platformdirs; per-project state lives under .atm/
...
def db_path() -> Path:
    return user_data_dir() / "atm.sqlite"
...
def project_atm_dir(project_root: Path) -> Path:
    return project_root / ".atm"


def project_manifest_path(project_root: Path) -> Path:
    return project_atm_dir(project_root) / "manifest.json"


def project_rules_dir(project_root: Path) -> Path:
    return project_atm_dir(project_root) / "rules"


def project_layout_profiles_dir(project_root: Path) -> Path:
    return project_atm_dir(project_root) / "layout-profiles"
