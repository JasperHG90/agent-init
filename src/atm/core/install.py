...
from atm.core import db, git, hashing, layout_profiles, manifest, paths, repos
from atm.core.models import InstalledSkill, Manifest, SkillIndex, SkillVersion

DEFAULT_TARGET_BASE = ".claude/skills"
_SNAPSHOT_SENTINEL = ".atm.complete"
...
def _warn_about_prereqs_and_capabilities(project_root: Path, qualified_name: str) -> None:
    ...
    from atm.core.skills import split_csv
    ...
    if missing:
        _install_warnings.append(
            ...
            "Install them with `atm skill install <name>`."
        )
