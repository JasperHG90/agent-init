...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--root", default=None, help="project root (default: $CLAUDE_PROJECT_DIR or cwd)"
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--validate", action="store_true", help="repair path if drifted; exit 1 if absent"
    )
    group.add_argument("--uninstall", action="store_true", help="remove the manifesto Stop hook")
